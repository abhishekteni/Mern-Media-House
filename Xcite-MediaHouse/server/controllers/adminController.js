...
const acceptJournalist = asyncHandler(async (req, res) => {
  ...
  if (journalist) {
    ...
    if (!journalist.isjournalist) {
      ...
      nodeMailSender(
        journalist.email,
        "Mern-MediaHouse : Congratulation, You Got Selected.",
        get_html_message(journalist.name)
      );
    } else {
      res.status(200).json({
        success: true,
...
const rejectJournalist = asyncHandler(async (req, res) => {
  ...
  if (journalist) {
    ...
    nodeMailSender(
      journalist.email,
      "Mern-MediaHouse : Sorry, You Are Not Selected.",
      get_html_message(journalist.name)
    );
    await Journalist.deleteOne({ _id: req.params.id })
      .then(() => {
        res.status(200).json({
          success: true,
          message: "Journalist Got Rejected",
        });
      })
      .catch(() => {
...
const getAllAcceptedJoun = asyncHandler(async (req, res) => {
  const Jouns = await Journalist.find({ isjournalist: true });

  const allJoun = [];

  for (let i = 0; i < Jouns.length; i++) {
    allJoun.push(Jouns[Jouns.length - (i + 1)]);
  }
...
const getAllPendingJoun = asyncHandler(async (req, res) => {
  const Jouns = await Journalist.find({ isjournalist: false });
...
const totalJounOfWeb = asyncHandler(async (req, res) => {
  ...
  if (journalist) {
    ...
    return res.status(404).json({
      success: false,
      error: "User is UnAthourised",
    });
  }
});

// Get all Joun
const getallJoun = asyncHandler(async (req, res) => {
  const Jouns = await Journalist.find();
