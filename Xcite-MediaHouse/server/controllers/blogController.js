...
const get4LatestBusinessBlogs = asyncHandler(async (req, res) => {
  const Blogs = await Blog.find({
    $or: [{ type: "business" }, { type: "Business" }],
  });
...
const get4LatestSociologyBlogs = asyncHandler(async (req, res) => {
  const Blogs = await Blog.find({
    $or: [{ type: "sociology" }, { type: "Sociology" }],
  });
...
const get4LatestEconomicBlogs = asyncHandler(async (req, res) => {
  const Blogs = await Blog.find({
    $or: [{ type: "economic" }, { type: "Economic" }],
  });
...
const getAllBlogs = asyncHandler(async (req, res) => {
  ...
  if (Blogs.length > 0) {
    ...
    for (let i = 0; i < Blogs.length; i++) {
      let data = {};
      const jounInfo = await Journalist.findById(
        Blogs[Blogs.length - (i + 1)].jounId
      );
      data = {
        blogInfo: Blogs[Blogs.length - (i + 1)],
        jounInfo: {
          _id: jounInfo._id,
          avatar: jounInfo.profilePicture,
          name: jounInfo.name,
        },
      };
...
const getBlogById = asyncHandler(async (req, res) => {
  ...
  if (foundBlog) {
    ...
    const data = {
      blogInfo: {
        foundBlog,
        thieUserLiked: liked.length > 0 ? true : false,
        thisUserSaved: save.length > 0 ? true : false,
      },
      jounInfo: {
        _id: journalist._id,
        avatar: journalist.profilePicture,
        name: journalist.name,
        subscriber: journalist.subscribe.length,
        thisUserSubscribed: subs.length > 0 ? true : false,
      },
    };
    res.status(200).json({
      success: true,
