...
const jounRegister = asyncHandler(async (req, res) => {
  ...
  if (!journalist) {
    ...
    if (newJoun) {
      ...
      nodeMailSender(
        email,
        "Mern-MediaHouse : Applied Successfull. ",
        get_html_message(name)
      );
    }
  } else {
...
const getJounById = asyncHandler(
...
const updateJounAvatar = asyncHandler(async (req, res) => {
  ...
  if (journalist.isjournalist) {
    const updateData = await Journalist.findOneAndUpdate(
      { _id: jounId },
      { $set: { profilePicture: avatarLink } }
    );
...
const updateJounEmail = asyncHandler(async (req, res) => {
  ...
  const journalist = await Journalist.findById(jounId);
  if (journalist.isjournalist) {
    const passwordMatched = await bycrpt.compare(
      currentPassword,
      journalist.password
    );
    if (passwordMatched) {
...
const forgetPasswordJoun = asyncHandler(async (req, res, next) => {
  ...
  if (journalist) {
    ...
    nodeMailSender(
      email,
      "Mern-MediaHouse : Password Recovery Link.",
...
const resetPasswordJoun = asyncHandler(async (req, res) => {
  ...
  if (journalist) {
    ...
    if (userAuth) {
      ...
      const updatePassword = await Journalist.findByIdAndUpdate(id, {
